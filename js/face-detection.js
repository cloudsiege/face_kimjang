...
// Function to compare face descriptors and return the most similar image filename
function findSimilarImage(faceDescriptors) {
  const threshold = 0.6; // Adjust this threshold as needed
  let mostSimilarFilename = null;
  let highestSimilarity = 0;

  for (const imageData of imageDatabase) {
    const similarity = faceapi.euclideanDistance(faceDescriptors, imageData.descriptors);

    if (similarity > threshold && similarity > highestSimilarity) {
      highestSimilarity = similarity;
      mostSimilarFilename = imageData.filename;
    }
  }

  return mostSimilarFilename;
}

// Array to store image data with descriptors and filenames
const imageDatabase = [
  // Add your image data here, e.g. { filename: "image1.jpg", descriptors: [...] }
  {filename: "JTY", 
  descriptors: [
  -0.10796964168548584,
  0.11063645780086517,
  ...
  -0.026122113689780235]}
];

// ...

// Inside the faceDetection interval function
...
// Check for similar images and log their filenames
for (const newFace of newFaces) {
  const similarImageFilename = findSimilarImage(newFace.descriptors);
  if (similarImageFilename) {
    console.log(`Found a similar image: ${similarImageFilename}`);
  }
}
